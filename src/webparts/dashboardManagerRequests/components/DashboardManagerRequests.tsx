import type { IDashboardManagerRequestsProps } from './IDashboardManagerRequestsProps';
import * as React from 'react';
import { DatePicker, mergeStyles, PrimaryButton, values } from '@fluentui/react';
import { buttonClassNames, Combobox, Dropdown ,Input,Option, OptionOnSelectData, SelectionEvents } from '@fluentui/react-components';
import {
  AttachRegular,
...
import { searchInSharePointList } from '../../../utils/search';
...
export default class DashboardManagerRequests extends React.Component<IDashboardManagerRequestsProps, {}> {
  ...
  public state={
    ...
    columns:["Nom","Motif d'absence",'Date début','Date fin','Statut','Action','Détails',''],
    fetcheditem: [] as any,
    open: false,
    statut: '',
    startDate:'',
  ...
  private updatStatus = async (item: any,newStatus:any) => {
    console.log(item.ID)
    const id = item.ID;
  ...
  private updatSoldeconge = async (item: any) => {
    console.log(item.ID)
    const id = item.ID_;
    await this.sp.web.lists.getByTitle("UsersList").items.getById(id).update({
      SoldConge: ( item.Solde - item.NB_days)
    });
    await this.sp.web.lists.getByTitle("leaveList").items.getById(item.ID).update({
      Solde: ( item.Solde - item.NB_days),
      UpdateDate : new Date()
  ...
  handleaccepted= async (item:any)=>{
    await this.updatStatus(item,'Acceptée');
    await this.updatSoldeconge(item)

  }

  handlerejected= async (item:any)=>{
    await this.updatStatus(item,'Rejetée');
  }
  ...
  handlefind=()=>{
    const listfound =searchInSharePointList(this.state.fetcheditem, this.state.keyword)
    console.log(listfound);
    this.setState({filteredItems:listfound})
  };
  ...
  public render(): React.ReactElement<IDashboardManagerRequestsProps> {
    ...
    return (
      ...
      <div style={{display:'flex',flexDirection:'row',gap:'20px',height:'35px'}}>
        ...
        <Input style={{width:'200px',marginTop:'10px'}} 
              ...
              placeholder=' ...'
              />
        <PrimaryButton onClick={this.handlefind} className={confirmButtonClass}>CHERCHER</PrimaryButton>
      </div>
          ...
          {this.state.filteredItems.map((item) => (
            <TableRow>
              ...
              <TableCell >
                <TableCellLayout>
                <div style={{display:'flex'}}>
                <PrimaryButton onClick={()=>this.handleaccepted(item)} style={{width:'90',height:'24px',backgroundColor:'#23365E' ,borderRadius:'25px',marginRight:'8px' }}>Accepter</PrimaryButton>
                <PrimaryButton onClick={()=>this.handlerejected(item)} style={{width:'90',height:'24px',backgroundColor:'#23365E' ,borderRadius:'25px',}}>Rejeter</PrimaryButton>
                </div>
                </TableCellLayout>
              </TableCell>
